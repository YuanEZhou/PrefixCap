...
def if_use_feat(caption_model):
    ...
    elif caption_model in  ['topdown', 'ClipCaptionPrefix', 'ClipCaption']:
...
def decode_sequence_gpt(tokenizer, seq):
    N, D = seq.size()
    out = []
    for i in range(N):
        mask = seq[i] > 0
        truncated_seq = seq[i][mask]
        txt = tokenizer.decode(truncated_seq)
        if int(os.getenv('REMOVE_BAD_ENDINGS', '0')):
            flag = 0
            words = txt.split(' ')
            for j in range(len(words)):
...
def grad_mask(optimizer, p_b=0,p_t=0, L=12, idx=0):
    for j, group in enumerate(optimizer.param_groups):
        # gpt in the second group.
        if j < 1:
            continue
        for k, param in enumerate(group['params']):
            if  (k)%12 == 0:
                if idx > (L-1):
                    idx = (L-1)
                p_i = p_b - (p_b-p_t)/(L-1) * idx
                idx += 1
                
            mask = 1/(1-p_i) * torch.bernoulli((torch.ones(param.shape)*(1-p_i)).to(param.device))
...
def get_std_opt(model, factor=1, warmup=2000, group=False):
    # return NoamOpt(model.tgt_embed[0].d_model, 2, 4000,
    #         torch.optim.Adam(model.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
    if group:
        return NoamOptGroup(model.gpt_embedding_size, factor, warmup,
            torch.optim.Adam([{'params':model.clip_project.parameters(), 'lr':0}, {'params':model.gpt.parameters(), 'lr':0}], betas=(0.9, 0.98), eps=1e-9))
    else:
        return NoamOpt(model.gpt_embedding_size, factor, warmup,
                torch.optim.Adam(model.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
