...
def build_dict(imgs, tokenizer,  params):
  ...
  for img in imgs:
    if (params['split'] == img['split']) or \
      ...
      for sent in img['sentences']:
        ...
        raw = sent['raw']
        idxs = tokenizer.encode(raw)
        idxs.append(0)
        ref_idxs.append(' '.join(map(str,idxs)))
...
def main(params):
  ...
  ngram_words, ngram_idxs, ref_len = build_dict(imgs, tokenizer, params)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--output_pkl', default='data/coco-train-clip-prefix', help='output pickle file')
