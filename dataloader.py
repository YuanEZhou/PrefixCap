...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        if self.opt.prefix_length > 0:
            mask_batch = np.concatenate((np.ones([data['labels'].shape[0], self.opt.prefix_length], dtype = 'float32'), mask_batch),axis=1)
    ...
    def __getitem__(self, index):
        ...
        if self.use_fc:
            fc_feat = self.fc_loader.get(str(self.info['images'][ix]['id'])).squeeze()
        else:
